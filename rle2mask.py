...
def files_rle2mask(csv, save_path):
    ...
    for line in open(csv, 'r', encoding="gbk").readlines()[1:]:
        arrs = line.split(',')
        name = arrs[0]
...
if __name__ == '__main__':
    ...
    mask_img_dir = os.path.join('Dataset','train', 'mask')
    pathlib.Path(mask_img_dir).mkdir(parents=True, exist_ok=True)
    files_rle2mask(mask_csv_filename, mask_img_dir)
